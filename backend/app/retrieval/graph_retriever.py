...
class GraphRetriever:
    ...
    def retrieve(
        ...
        relationship_candidates = self.relationship_resolver.resolve(relation)
        ...
        entities_with_candidates = [
            entity for entity in linked_entities if entity["candidates"]
        ]
        ...
        if len(entities_with_candidates) >= 2:
            raw_facts = self._retrieve_between_entities(
                entities_with_candidates,
                best_relationship,
            )
        else:
            raw_facts = self._retrieve_from_entity(
